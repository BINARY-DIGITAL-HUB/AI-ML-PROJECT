...
if update_button:
    ...
    if social_choice == 'Facebook':
        ...
        with st.spinner('Updating Corpus......'):  
            ...
            python_file_path = './minner_tool.py'
            # Define the parameters you want to pass to the Python file
            parameters = [email_input, password_input]
            # Run the Python file using subprocess and pass the parameters
            minin_status = subprocess.run(['python', python_file_path] + parameters)
            
            if minin_status.returncode == 0: 
                st.success("Mining Complete")
            else: 
                st.error("Unable to Complete Mining")
...
if login_button: 
    ...
    login_detials = pd.read_sql_table('credential_table', connection)
