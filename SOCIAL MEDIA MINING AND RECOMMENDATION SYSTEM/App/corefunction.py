...
# getting contact and email pattern from post...
def contact_email_detector(posts, is_post=True):
    contact_diction = {'id':[], 'contacts':[] , 'emails':[],'contact_len':[], 'email_len':[]}
    counter = 0
    for post in posts: 
        # post = post.split(' ')
        # print(post)
        ...
        if is_post:
            contact_diction['id'].append('Post {}'.format(counter))
        else : 
            contact_diction['id'].append('Comment {}'.format(counter))
