...
def load_facebook_post_comment(post_data):
    data = pd.read_csv(post_data)
    return data

# def get_facebook_comment(comment_data):
#     comment = pd.read_csv(comment_data)
#     return
...
with dash_layout:
    ...
    #     # display_profile_data(profile_info)
    #     POST = load_facebook_post_comment('user_post_data.csv')
...
with post_layout: 
    ...
    with st.expander('Email, Contact & Address Detector'):
        ...
        print(post_text)
        cm = load_facebook_post_comment('comment_table.csv')
        
        # image_post = post.images_description.values[::-1]

         
            
        post_data_detection = corefunc.contact_email_detector(post_text)
        # image_data_detection = corefunc.contact_email_detector(image_post)
        # comment_data_detection = corefunc.contact_email_detector(cm.comment_text.values, False)

        # st.write(contact_data_detection)
        # st.write(image_data_detection)
        
        # lay_contact , lay_email = st.columns(2)
        post_option_chk = st.checkbox('Post')
        if post_option_chk: 
        #    display_contac_match_content(pd.DataFrame(post_data_detection))
            st.write(pd.DataFrame(post_data_detection))
